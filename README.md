...
> `oa` 会以当前目录为工作目录，文件读写、命令执行等操作都会限制在该目录下。首次运行或配置缺失时会自动打开配置向导，引导你配置供应商和模型，保存到 `~/.openagent/config.json`。支持配置多个供应商，每个供应商有独立的 `baseUrl`、`apiKey` 和 `models` 列表，通过 `activeModel`（格式 `"供应商名/模型名"`）指定当前使用的模型。
