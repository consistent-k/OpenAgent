...
│       │   ├── adapter.ts                 # 消息适配器
│       │   ├── send.ts                    # 消息发送
│       │   ├── markdown-filter.ts         # Markdown → 微信纯文本
│       │   ├── context-token.ts           # 上下文 token 持久化
│       │   ├── sync-buf.ts                # getUpdates 游标持久化
│       │   ├── logger.ts                  # 文件日志
│       │   ├── redact.ts                  # 日志脱敏
│       │   ├── random.ts                  # ID 生成
│       │   ├── session.ts                 # 重导出 SessionManager
│       │   ├── plugin-types.ts            # 依赖注入类型
│       │   ├── types.ts                   # iLink 协议类型
│       │   └── vendor.d.ts                # 第三方类型声明
│       ├── package.json
│       ├── tsconfig.json
...
所有工具在 `packages/core/src/agent/tools/index.ts` 中注册：
...
send(text)
  │
  ├── 1. 检查 isConfigReady() → 未配置则显示错误提示并返回
  ├── 2. 追加用户消息 + 空 AI 消息到 displayMessages
  ├── 3. 展开 @mentions → expandMentions(text, fileIndex, cwd)
  ├── 4. 过滤掉 system 消息，追加用户消息到 messages
  └── 5. streamMessages(messages)
           │
...
```
优先级 1: pendingApproval 不为 null → 'approval'
优先级 2: sessionPicker 不为空 → 'session'
优先级 3: themePickerOpen → 'theme'
优先级 4: configPickerOpen → 'config'
优先级 5: disabled → 'disabled'
优先级 6: 输入以 '/' 开头且无空格 → 'command'
优先级 7: @mention 激活中 → 'file'
优先级 8: 默认 → 'text'
```
...
```
工具调用分组规则 (groupToolParts.ts):

可折叠工具 (Collapsible) ← 连续出现时折叠为 ToolCallGroup
  ├── read_file
  ├── read_directory
  ├── grep
  ├── glob
...
不可折叠工具 (Individual) ← 始终单独渲染
  ├── edit_file
  ├── write_file
  ├── execute_bash
  └── ask_user_question

分组逻辑：连续的可折叠工具合并为一个 ToolCallGroup（中间跳过 reasoning 片段）
  Ctrl+O 切换展开/折叠状态
```
...
**存储路径**：`~/.openagent/sessions/{projectName}-{hash}+{branch}/`

- 会话文件：`{name}.json`（name 为 ISO 时间戳，`:` 和 `.` 替换为 `-`）
- 索引文件：`index.json`（SessionSummary 数组，用于快速列表）
- 旧目录格式 `{projectName}+{branch}` 自动迁移到新格式（添加 hash 防冲突）
...
└───────────┬─────────────────────────────────────────────────────┘
    ...
    └───────┬───────┘             │ findCommand()          │
            ...
            ▼                     └────────────────────────┘
    ┌───────────────────┐
    │ isConfigReady()    │ ← 否 → 提示"配置未完善，请 /config"
    └───────┬───────────┘
            │ 是
            ▼
    ┌───────────────┐
...
ask_user_question    其他工具
 ...
 选择选项   Esc       批准    始终批准   拒绝
  │         │          │         │      │
  ▼         ▼          ▼         ▼      ▼
selectQ.  denyP.   approveP.  alwaysA. denyP.
  │         │          │        │       │
  │         │          │        │       │
  │         │          │  写入偏好到     │
  │         │          │  approvals/     │
...
1. 在 `packages/core/src/agent/tools/` 下创建新文件
...
3. 在 `packages/core/src/agent/tools/index.ts` 的 `tools` 对象中注册
...
主应用
├── config.json → channels 字段
├── /channel 命令 → 动态加载插件
└── 依赖注入 → runAgent, enableAutoApprove
```
