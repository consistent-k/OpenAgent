...
export const zh: Record<string, string> = {
    ...
    // ── 状态栏 ──
    'status.header.indexing': '索引中...',
    'status.header.indexError': '索引错误',
    'status.header.fileCount': '{count} 个文件',
    ...
    'status.bar.outputLabel': '输出：',
    'status.bar.noUsage': '无用量',

    // ── 工具状态 ──
    'tool.state.waitingInput': '等待输入',
    'tool.state.pending': '等待中',
    'tool.state.awaitingApproval': '等待审批',
    'tool.state.executing': '执行中',
    'tool.state.error': '错误',
    ...
    'tool.result.denied': '已拒绝',
    'tool.result.checkmark': '✓',
    'tool.result.moreLines': '...（还有 {count} 行）',

    'tool.verb.reading': '读取中',
    'tool.verb.read': '已读取',
    'tool.verb.file': '文件',
    'tool.verb.files': '文件',
    'tool.verb.listing': '列取中',
    'tool.verb.listed': '已列取',
    'tool.verb.directory': '目录',
    'tool.verb.directories': '目录',
    'tool.verb.searching': '搜索中',
    'tool.verb.searched': '已搜索',
    'tool.verb.pattern': '模式',
    'tool.verb.patterns': '模式',
    'tool.verb.finding': '查找中',
    'tool.verb.found': '已找到',
    'tool.verb.fetching': '获取中',
    'tool.verb.fetched': '已获取',
    'tool.verb.url': 'URL',
    'tool.verb.urls': 'URL',
    'tool.verb.query': '查询',
    ...
    // ── 工具错误消息 ──
    'tool.bash.dangerRefused': '拒绝执行危险命令（{reason}）：{command}',
    'tool.bash.outputTruncated': '...（输出已截断，省略 {kb}KB）',

    'tool.editFile.sameStrings': 'old_string 和 new_string 不能相同',
    'tool.editFile.notFound': '在文件中未找到 old_string：{filePath}',
    'tool.editFile.multipleMatches': 'old_string 在文件中出现 {count} 次，请使用 replace_all 或提供更精确的 old_string',

    'tool.readFile.invalidRange': 'endLine 必须 >= startLine',
    'tool.readFile.notAFile': '路径不是文件：{filePath}',
    'tool.readFile.fileTooLarge': '文件过大（{size}KB），超过 {limit}KB 限制',
    'tool.readFile.startLineExceeds': 'startLine {startLine} 超过文件总行数 {totalLines}',

    'tool.glob.absolutePath': 'glob 模式必须是相对路径',
    'tool.glob.parentRef': 'glob 模式不能包含 ".."',

    'tool.grep.invalidRegex': '无效的正则表达式：{error}',
    'tool.grep.cannotReadFile': '无法读取文件：{path}',
    'tool.grep.pathNotExists': '路径不存在或不是文件/目录：{path}',

    'tool.fetch.unsupportedProtocol': '仅支持 http 和 https 协议',
    'tool.fetch.localhostBlocked': '不允许请求 localhost',
    'tool.fetch.privateAddressBlocked': '不允许请求私有/回环地址',
    'tool.fetch.resolvesToPrivate': '主机名解析到私有/回环地址',
    'tool.fetch.fetchFailed': '获取 URL 失败：{error}',

    'tool.writeFile.fileAlreadyExists': '文件已存在：{filePath}',
    'tool.writeFile.notAFile': '路径不是文件：{filePath}',

    'tool.webSearch.apiNotConfigured': '未配置搜索 API',
    ...
    'error.streamInterrupted': '（流中断 — 工具结果未收到）',
